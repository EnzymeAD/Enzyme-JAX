...
module {
  // CPU:  func.func @test_select_trace(%arg0: tensor<i1>) -> tensor<ui64> {
  ...
  // CPU-NEXT:  }
  func.func @test_select_trace(%condition: tensor<i1>) -> !enzyme.Trace {
    %trace_true = enzyme.initTrace : !enzyme.Trace
    %trace_false = enzyme.initTrace : !enzyme.Trace
    %selected = enzyme.selectTrace %condition, %trace_true, %trace_false : tensor<i1>
