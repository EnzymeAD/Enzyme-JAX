...
// ============================================================================
// Test: DotAbsorbScatter — absorb scatter-into-zeros by gathering from LHS
// ============================================================================
//
// Model: dot_general(A:[2,4], reshape(scatter(zeros:[2,2], idx, data:[2])) -> [4,1])
//        where A and idx are sample-invariant.
// SICM gathers columns from A: gathered_A = gather(A, linear_idx) : [2,2]
// Then hoists gathered_A outside mcmc_region. The dot shrinks from [2,4] to [2,2].

// CHECK-LABEL: func.func @test_dot_absorb_scatter
// CHECK-SAME: %[[RNG:.*]]: tensor<2xui64>, %[[PRIOR:.*]]: tensor<f64>, %[[A:.*]]: tensor<2x4xf64>, %[[IDX:.*]]: tensor<2x2xi32>, %[[TRACE:.*]]: tensor<1x1xf64>
...
module {
  ...
  func.func @test_dot_absorb_scatter(%rng : tensor<2xui64>, %prior : tensor<f64>,
                                      %A : tensor<2x4xf64>, %idx : tensor<2x2xi32>,
                                      %trace : tensor<1x1xf64>)
      -> (tensor<1x1xf64>, tensor<1xi1>, tensor<2xui64>) {
    %step_size = arith.constant dense<0.1> : tensor<f64>

    %result:8 = enzyme.mcmc @model_scatter(%rng, %prior, %A, %idx) given %trace
        step_size = %step_size {
      ...
      thinning = 1 : i64,
      num_warmup = 0 : i64
    } : (tensor<2xui64>, tensor<f64>, tensor<2x4xf64>, tensor<2x2xi32>, tensor<1x1xf64>, tensor<f64>)
  ...
  func.func @model_scatter(%rng : tensor<2xui64>, %prior : tensor<f64>,
                            %A : tensor<2x4xf64>, %idx : tensor<2x2xi32>)
      -> (tensor<2xui64>, tensor<f64>) {
    ...
    // Reshape to [4, 1]
    %reshaped = stablehlo.reshape %scattered : (tensor<2x2xf64>) -> tensor<4x1xf64>
