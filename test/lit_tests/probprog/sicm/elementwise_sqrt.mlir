...
// ============================================================================
// Test: SqrtScaleFactorization — sqrt(broadcast(s) * A) -> broadcast(sqrt(s)) * sqrt(A)
// ============================================================================

// CHECK-LABEL: func.func @test_sqrt_scale
// CHECK-SAME: %[[RNG:.*]]: tensor<2xui64>, %[[PRIOR:.*]]: tensor<f64>, %[[A:.*]]: tensor<3xf64>, %[[TRACE:.*]]: tensor<1x1xf64>
// sqrt(A) should be hoisted before mcmc_region
// CHECK: stablehlo.sqrt %[[A]] : tensor<3xf64>
// CHECK: enzyme.mcmc_region
// Inside: sqrt(scale) broadcast and multiply
// CHECK: enzyme.sample_region
// CHECK: stablehlo.sqrt %{{.*}} : tensor<f64>
...
module {
  ...
  func.func @test_sqrt_scale(%rng : tensor<2xui64>, %prior : tensor<f64>,
                              %A : tensor<3xf64>, %trace : tensor<1x1xf64>)
      -> (tensor<1x1xf64>, tensor<1xi1>, tensor<2xui64>) {
    %step_size = arith.constant dense<0.1> : tensor<f64>

    %result:3 = enzyme.mcmc @model_sqrt(%rng, %prior, %A) given %trace
        step_size = %step_size {
  ...
  func.func @model_sqrt(%rng : tensor<2xui64>, %prior : tensor<f64>,
                         %A : tensor<3xf64>)
      -> (tensor<2xui64>, tensor<f64>) {
    ...
    %sqrt_result = stablehlo.sqrt %scaled : tensor<3xf64>

    %slice = "stablehlo.slice"(%sqrt_result) {start_indices = array<i64: 0>, limit_indices = array<i64: 1>, strides = array<i64: 1>} : (tensor<3xf64>) -> tensor<1xf64>
