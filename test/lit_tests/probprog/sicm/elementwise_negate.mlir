...
// ============================================================================
// Test: NegateDistribution — negate(broadcast(s) * A) -> broadcast(negate(s)) * A
// ============================================================================

// CHECK-LABEL: func.func @test_negate_scale
// CHECK-SAME: %[[RNG:.*]]: tensor<2xui64>, %[[PRIOR:.*]]: tensor<f64>, %[[A:.*]]: tensor<3xf64>, %[[TRACE:.*]]: tensor<1x1xf64>
// CHECK: enzyme.mcmc_region
// Inside: negate(scale), broadcast, multiply — no stablehlo.negate of a tensor<3xf64>
// CHECK: enzyme.sample_region
// CHECK: stablehlo.negate %{{.*}} : tensor<f64>
// CHECK: stablehlo.broadcast_in_dim
// CHECK-NEXT: stablehlo.multiply
// CHECK-NOT: stablehlo.negate %{{.*}} : tensor<3xf64>
// CHECK: enzyme.yield
...
module {
  ...
  func.func @test_negate_scale(%rng : tensor<2xui64>, %prior : tensor<f64>,
                                %A : tensor<3xf64>, %trace : tensor<1x1xf64>)
      -> (tensor<1x1xf64>, tensor<1xi1>, tensor<2xui64>) {
    %step_size = arith.constant dense<0.1> : tensor<f64>

    %result:3 = enzyme.mcmc @model_negate(%rng, %prior, %A) given %trace
        step_size = %step_size {
  ...
  func.func @model_negate(%rng : tensor<2xui64>, %prior : tensor<f64>,
                           %A : tensor<3xf64>)
      -> (tensor<2xui64>, tensor<f64>) {
    ...
    %neg_result = stablehlo.negate %scaled : tensor<3xf64>

    %slice = "stablehlo.slice"(%neg_result) {start_indices = array<i64: 0>, limit_indices = array<i64: 1>, strides = array<i64: 1>} : (tensor<3xf64>) -> tensor<1xf64>
