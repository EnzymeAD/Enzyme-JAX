...
// ============================================================================
// Test: AbsScaleFactorization — abs(broadcast(s) * A) -> broadcast(abs(s)) * abs(A)
// ============================================================================

// CHECK-LABEL: func.func @test_abs_scale
// CHECK-SAME: %[[RNG:.*]]: tensor<2xui64>, %[[PRIOR:.*]]: tensor<f64>, %[[A:.*]]: tensor<3xf64>, %[[TRACE:.*]]: tensor<1x1xf64>
// abs(A) should be hoisted before mcmc_region
// CHECK: stablehlo.abs %[[A]] : tensor<3xf64>
// CHECK: enzyme.mcmc_region
// Inside: abs(scale) broadcast and multiply
// CHECK: enzyme.sample_region
// CHECK: stablehlo.abs %{{.*}} : tensor<f64>
...
module {
  ...
  func.func @test_abs_scale(%rng : tensor<2xui64>, %prior : tensor<f64>,
                             %A : tensor<3xf64>, %trace : tensor<1x1xf64>)
      -> (tensor<1x1xf64>, tensor<1xi1>, tensor<2xui64>) {
    %step_size = arith.constant dense<0.1> : tensor<f64>

    %result:8 = enzyme.mcmc @model_abs(%rng, %prior, %A) given %trace
        step_size = %step_size {
  ...
  func.func @model_abs(%rng : tensor<2xui64>, %prior : tensor<f64>,
                        %A : tensor<3xf64>)
      -> (tensor<2xui64>, tensor<f64>) {
    ...
    %abs_result = stablehlo.abs %scaled : tensor<3xf64>

    %slice = "stablehlo.slice"(%abs_result) {start_indices = array<i64: 0>, limit_indices = array<i64: 1>, strides = array<i64: 1>} : (tensor<3xf64>) -> tensor<1xf64>
