...
// ============================================================================
// Test: RsqrtScaleFactorization — rsqrt(broadcast(s) * A) -> broadcast(rsqrt(s)) * rsqrt(A)
// ============================================================================

// CHECK-LABEL: func.func @test_rsqrt_scale
// CHECK-SAME: %[[RNG:.*]]: tensor<2xui64>, %[[PRIOR:.*]]: tensor<f64>, %[[A:.*]]: tensor<3xf64>, %[[TRACE:.*]]: tensor<1x1xf64>
// rsqrt(A) should be hoisted before mcmc_region
// CHECK: stablehlo.rsqrt %[[A]] : tensor<3xf64>
// CHECK: enzyme.mcmc_region
// Inside: rsqrt(scale) broadcast and multiply
// CHECK: enzyme.sample_region
// CHECK: stablehlo.rsqrt %{{.*}} : tensor<f64>
...
module {
  ...
  func.func @test_rsqrt_scale(%rng : tensor<2xui64>, %prior : tensor<f64>,
                               %A : tensor<3xf64>, %trace : tensor<1x1xf64>)
      -> (tensor<1x1xf64>, tensor<1xi1>, tensor<2xui64>) {
    %step_size = arith.constant dense<0.1> : tensor<f64>

    %result:3 = enzyme.mcmc @model_rsqrt(%rng, %prior, %A) given %trace
        step_size = %step_size {
  ...
  func.func @model_rsqrt(%rng : tensor<2xui64>, %prior : tensor<f64>,
                          %A : tensor<3xf64>)
      -> (tensor<2xui64>, tensor<f64>) {
    ...
    %rsqrt_result = stablehlo.rsqrt %scaled : tensor<3xf64>

    %slice = "stablehlo.slice"(%rsqrt_result) {start_indices = array<i64: 0>, limit_indices = array<i64: 1>, strides = array<i64: 1>} : (tensor<3xf64>) -> tensor<1xf64>
