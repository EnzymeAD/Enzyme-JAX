...
// ============================================================================
// Test: DotAbsorbFFT — absorb FFT from RHS into LHS matrix
// ============================================================================
//
// Model: dot_general(A:[2,4], reshape(fft(X:[2,2])) -> [4,1])
//        where A is sample-invariant.
// SICM absorbs the FFT into A: A' = reshape(fft(reshape(A,[2,2,2])),[2,4])
// Then hoists the pre-FFT'd matrix A' outside mcmc_region.

// CHECK-LABEL: func.func @test_dot_absorb_fft
// CHECK-SAME: %[[RNG:.*]]: tensor<2xui64>, %[[PRIOR:.*]]: tensor<f64>, %[[A:.*]]: tensor<2x4xcomplex<f64>>, %[[TRACE:.*]]: tensor<1x1xf64>
// The FFT of A should be hoisted before mcmc_region
// CHECK: stablehlo.reshape %[[A]] : (tensor<2x4xcomplex<f64>>) -> tensor<2x2x2xcomplex<f64>>
// CHECK-NEXT: stablehlo.fft
// CHECK-NEXT: stablehlo.reshape
// CHECK-NOT: stablehlo.fft
// CHECK: enzyme.mcmc_region
// Inside: no fft, just dot_general with pre-fft'd matrix
...
module {
  ...
  func.func @test_dot_absorb_fft(%rng : tensor<2xui64>, %prior : tensor<f64>,
                                  %A : tensor<2x4xcomplex<f64>>,
                                  %trace : tensor<1x1xf64>)
      -> (tensor<1x1xf64>, tensor<1xi1>, tensor<2xui64>) {
    %step_size = arith.constant dense<0.1> : tensor<f64>

    %result:3 = enzyme.mcmc @model_fft(%rng, %prior, %A) given %trace
        step_size = %step_size {
  ...
  func.func @model_fft(%rng : tensor<2xui64>, %prior : tensor<f64>,
                        %A : tensor<2x4xcomplex<f64>>)
      -> (tensor<2xui64>, tensor<f64>) {
    ...
    // Build a 2x2 complex matrix from the sample
    ...
    // FFT the matrix
    %fft_result = stablehlo.fft %x_mat, type = FFT, length = [2, 2] : (tensor<2x2xcomplex<f64>>) -> tensor<2x2xcomplex<f64>>

    // Reshape to [4, 1] for dot
    ...
    // Extract real part of first element via stablehlo ops
