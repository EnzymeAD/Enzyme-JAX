...
// SPMD-NEXT:    %1:3 = stablehlo.custom_call @_SPMDInternalOp_MultiRotate(%0) {backend_config = "dimension=2,left_amount=2,right_amount=0", sdy.sharding = #sdy.sharding_per_value<[<@mesh1, [{"z"}, {"y"}, {"x"}]>, <@mesh1, [{"z"}, {"y"}, {"x"}]>, <@mesh1, [{"z"}, {"y"}, {"x"}]>]>} : (tensor<4x8x80xf64>) -> (tensor<4x8x80xf64>, tensor<4x8x80xf64>, tensor<4x8x80xf64>)
...
// SPMD-NEXT:    %0 = stablehlo.slice %arg0 [8:12, 8:16, 8:88] {sdy.sharding = #sdy.sharding_per_value<[<@mesh1, [{"z"}, {"y"}, {"x"}]>]>} : (tensor<20x24x96xf64>) -> tensor<4x8x80xf64>
// SPMD-NEXT:    %1:3 = stablehlo.custom_call @_SPMDInternalOp_MultiRotate(%0) {backend_config = "dimension=1,left_amount=2,right_amount=0", sdy.sharding = #sdy.sharding_per_value<[<@mesh1, [{"z"}, {"y"}, {"x"}]>, <@mesh1, [{"z"}, {"y"}, {"x"}]>, <@mesh1, [{"z"}, {"y"}, {"x"}]>]>} : (tensor<4x8x80xf64>) -> (tensor<4x8x80xf64>, tensor<4x8x80xf64>, tensor<4x8x80xf64>)
// SPMD-NEXT:    return %1#0 : tensor<4x8x80xf64>
// SPMD-NEXT:  }
