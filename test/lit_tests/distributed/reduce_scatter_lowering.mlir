...
module {
  ...
  distributed.Function @collective context %mesh_in : !axis.factor_group<8> arg_types [tensor<8xf32>] ret_types [tensor<2xf32>] {
  ^bb0(%arg0: tensor<8xf32>):
    %h = distributed.Collective %arg0 : tensor<8xf32> on %mesh_in : !axis.factor_group<8> to tensor<2xf32> on %mesh_out : !axis.factor_group<8> reduces (%reduction @add) : !axis.factor_group<4> maps %mapping : !axis.map
    %v = distributed.Await %h : !distributed.asynch_handle<tensor<2xf32>> -> tensor<2xf32>
    distributed.DistributedYield %v tensor<2xf32>
  }
}
