...
module {
  ...
  %f0 = axis.factor %ax : !axis.shape_axis<tensor<4xf32>, 0> <4, 1>
  %ctx = axis.product %f0 : !axis.axis_factor<!axis.shape_axis<tensor<4xf32>, 0>, 4, 1>
  %rep = "axis.product"() : () -> !axis.factor_group<1>

  distributed.Function @callee_mod context %ctx : !axis.factor_group<4> arg_types [i32] ret_types [i32] {
  ^bb0(%arg0: i32):
    distributed.DistributedYield %arg0 i32
  }

  distributed.Function @caller_mod context %ctx : !axis.factor_group<4> arg_types [i32] ret_types [i32] {
  ^bb0(%arg0: i32):
    %r = distributed.DistributedCall @callee_mod replicate_over %rep %arg0 : !axis.factor_group<1>, i32 -> i32
