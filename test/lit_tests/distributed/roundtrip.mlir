...
module {
  ...
  distributed.MeshComputation @mc mesh @mesh0 {
    ...
    %lf0 = axis.factor %l0 [2] : !distributed.logical_mesh_axis<2>
    %lf1 = axis.factor %l1 [2] : !distributed.logical_mesh_axis<2>
    %ta = axis.getaxis tensor<8xf32> 0
    %ta_out = axis.getaxis tensor<4xf32> 0
    %tf_out = axis.factor %ta_out [4] : !axis.shape_axis<tensor<4xf32>, 0>
    %tf_to_mesh, %tf_remain = axis.factor %ta [4, 2] : !axis.shape_axis<tensor<8xf32>, 0>
    ...
    ^bb0(%arg0: tensor<8xf32>):
      %h = distributed.Collective %arg0 : tensor<8xf32> on %mesh_in : !axis.factor_group<4> to tensor<4xf32> on %mesh_out : !axis.factor_group<4> reduces (%reduction @add) : !axis.factor_group<2> maps (%lhs_group_1 -> %mesh_out, %lhs_group_2 -> %rhs_group_2) : [!axis.factor_group<4>, !axis.factor_group<4>] [!axis.factor_group<4>, !axis.factor_group<4>]
