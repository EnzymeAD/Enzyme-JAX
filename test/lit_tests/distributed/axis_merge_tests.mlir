...
// RUN: enzymexlamlir-opt --sdy-propagation-pipeline --sdy-insert-explicit-reshards --convert-main-to-distributed-function --materialize-distributed-collectives --sdy-drop-sharding-and-mesh --cluster-distributed-kernels="dump-logical-axes=true" -split-input-file -o /dev/null %s 2>&1 | FileCheck %s
// RUN: enzymexlamlir-opt --sdy-propagation-pipeline --sdy-insert-explicit-reshards --convert-main-to-distributed-function --materialize-distributed-collectives --sdy-drop-sharding-and-mesh --cluster-distributed-kernels="dump-logical-axes=true" --mlir-print-ir-after=cluster-distributed-kernels -split-input-file -o /dev/null %s 2>&1 | FileCheck %s --check-prefix=CHAIN
...
// RUN: enzymexlamlir-opt --sdy-propagation-pipeline --sdy-insert-explicit-reshards --convert-main-to-distributed-function --materialize-distributed-collectives --sdy-drop-sharding-and-mesh --canonicalize --cse --cluster-distributed-kernels --canonicalize --stabilize-axis-order -split-input-file %s -o - > %t.explicit
...
// RUN: enzymexlamlir-opt --materialize-distributed-collectives --sdy-drop-sharding-and-mesh --canonicalize --cse -split-input-file %t.stage2 -o %t.stage3
