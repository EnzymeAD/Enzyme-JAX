...
// PDLL-GLOBAL-NEXT:   %[[slice2:.*]] = stablehlo.slice %[[extend:.*]] [0:1, 1:3, 0:96] : (tensor<1x10x96xf64>) -> tensor<1x2x96xf64>
// PDLL-GLOBAL-NEXT:   %[[slice1:.*]] = stablehlo.slice %[[extend:.*]] [0:1, 0:10, 0:96] : (tensor<1x10x96xf64>) -> tensor<1x10x96xf64>
// PDLL-GLOBAL-NEXT:   return %[[slice2:.*]], %[[slice1:.*]], %[[extend:.*]] : tensor<1x2x96xf64>, tensor<1x10x96xf64>, tensor<1x10x96xf64>
// PDLL-GLOBAL-NEXT: }
...
// PDLL-LOCAL-NEXT:   %[[slice2:.*]] = stablehlo.slice %[[extend1:.*]] [0:1, 1:3, 0:96] : (tensor<1x10x96xf64>) -> tensor<1x2x96xf64>
// PDLL-LOCAL-NEXT:   %[[slice1:.*]] = stablehlo.slice %[[extend2:.*]] [0:1, 0:10, 0:96] : (tensor<1x10x96xf64>) -> tensor<1x10x96xf64>
// PDLL-LOCAL-NEXT:   %[[extend3:.*]] = "enzymexla.extend"(%arg0) <{dimension = 2 : i64, lhs = 7 : i64, rhs = 9 : i64}> : (tensor<1x10x80xf64>) -> tensor<1x10x96xf64>
// PDLL-LOCAL-NEXT:   return %[[slice2:.*]], %[[slice1:.*]], %[[extend3:.*]] : tensor<1x2x96xf64>, tensor<1x10x96xf64>, tensor<1x10x96xf64>
// PDLL-LOCAL-NEXT: }
