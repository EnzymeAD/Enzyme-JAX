...
func.func @test_dot_general_propagate(%arg0: tensor<2x2x3xf32> {enzymexla.partial_symmetry = [#enzymexla.partial_symmetry<<[0, 1]>>]}) -> tensor<2x2x2x2xf32> { 
  ...
  %tmp = stablehlo.broadcast_in_dim %1, dims = [0, 1] : (tensor<2x2xf32>) -> tensor<2x2x3xf32>
  %2 = stablehlo.dot_general %arg0, %tmp, contracting_dims = [2] x [2] : (tensor<2x2x3xf32>, tensor<2x2x3xf32>) -> tensor<2x2x2x2xf32> 
  return %2 : tensor<2x2x2x2xf32>
}
// CHECK: func.func @test_dot_general_propagate(%arg0: tensor<2x2x3xf32> {enzymexla.partial_symmetry = [#enzymexla.partial_symmetry<<[0, 1]>>]}) -> tensor<2x2x2x2xf32> {
...
// CHECK-NEXT: %1 = stablehlo.dot_general %0, %0, contracting_dims = [1] x [0] {enzymexla.partial_symmetry = [#enzymexla.partial_symmetry<<[0, 1]>>]} : (tensor<2x2xf32>, tensor<2x2xf32>) -> tensor<2x2xf32>
// CHECK-NEXT: %2 = stablehlo.broadcast_in_dim %1, dims = [0, 1] {enzymexla.partial_symmetry = [#enzymexla.partial_symmetry<<[0, 1]>>]} : (tensor<2x2xf32>) -> tensor<2x2x3xf32>
// CHECK-NEXT: %3 = stablehlo.dot_general %arg0, %2, contracting_dims = [2] x [2] {enzymexla.partial_symmetry = [#enzymexla.partial_symmetry<<[0, 1]>, <[2, 3]>>]} : (tensor<2x2x3xf32>, tensor<2x2x3xf32>) -> tensor<2x2x2x2xf32>
// CHECK-NEXT: return %3 : tensor<2x2x2x2xf32>
// CHECK-NEXT: }
