...
// CPU-LABEL:  llvm.func @enzymexla_wrapper_blas_strsm_
// CPU-LABEL:  llvm.func @enzymexla_blas_strsm_
...
// CPU-NEXT:    %0 = enzymexla.jit_call @enzymexla_wrapper_blas_strsm_ (%c, %c_0, %c_1, %c_1, %c_2, %c_2, %arg0, %arg1, %c_2, %arg2, %c_2) {operand_layouts = [dense<> : tensor<0xindex>, dense<> : tensor<0xindex>, dense<> : tensor<0xindex>, dense<> : tensor<0xindex>, dense<> : tensor<0xindex>, dense<> : tensor<0xindex>, dense<> : tensor<0xindex>, dense<[0, 1]> : tensor<2xindex>, dense<> : tensor<0xindex>, dense<[0, 1]> : tensor<2xindex>, dense<> : tensor<0xindex>], output_operand_aliases = [#stablehlo.output_operand_alias<output_tuple_indices = [], operand_index = 9, operand_tuple_indices = []>], result_layouts = [dense<[0, 1]> : tensor<2xindex>], xla_side_effect_free} : (tensor<i8>, tensor<i8>, tensor<i8>, tensor<i8>, tensor<i64>, tensor<i64>, tensor<f32>, tensor<64x64xf32>, tensor<i64>, tensor<64x64xf32>, tensor<i64>) -> tensor<64x64xf32>
