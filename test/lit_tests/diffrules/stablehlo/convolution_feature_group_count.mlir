...
// REVERSE-NEXT{LITERAL}:    %1 = stablehlo.reverse %0, dims = [0, 1] : tensor<3x3x4x2xf32>
// REVERSE-NEXT{LITERAL}:    %2 = stablehlo.convolution(%arg2, %1) dim_numbers = [b, f, 1, 0]x[0, 1, o, i]->[b, f, 1, 0], window = {stride = [1, 1], pad = [[2, 2], [2, 2]], lhs_dilate = [1, 1], rhs_dilate = [1, 1]} {batch_group_count = 1 : i64, feature_group_count = 4 : i64} : (tensor<1x8x2x2xf32>, tensor<3x3x4x2xf32>) -> tensor<1x4x4x4xf32>
// REVERSE-NEXT{LITERAL}:    %3 = stablehlo.convolution(%arg0, %arg2) dim_numbers = [f, b, 1, 0]x[i, o, 1, 0]->[0, 1, b, f], window = {stride = [1, 1], pad = [[0, 0], [0, 0]], rhs_dilate = [1, 1]} {batch_group_count = 4 : i64, feature_group_count = 1 : i64} : (tensor<1x4x4x4xf32>, tensor<1x8x2x2xf32>) -> tensor<3x3x1x8xf32>
// REVERSE-NEXT{LITERAL}:    return %2, %3 : tensor<1x4x4x4xf32>, tensor<3x3x1x8xf32>
