...
// CHECK-NEXT:     %15 = stablehlo.dot_general %14, %5, batching_dims = [2] x [2], contracting_dims = [1] x [1], precision = [DEFAULT, DEFAULT] : (tensor<16x5x16xcomplex<f32>>, tensor<4x5x16xcomplex<f32>>) -> tensor<16x16x4xcomplex<f32>>
// CHECK-NEXT:     %16 = chlo.conj %15 : tensor<16x16x4xcomplex<f32>> -> tensor<16x16x4xcomplex<f32>>
// CHECK-NEXT:     %17 = stablehlo.transpose %16, dims = [0, 2, 1] : (tensor<16x16x4xcomplex<f32>>) -> tensor<16x4x16xcomplex<f32>>
...
// CHECK-NEXT:     %20 = stablehlo.pad %19, %cst_0, low = [0, 0, 0], high = [0, 0, 497], interior = [0, 0, 0] : (tensor<4x5x16xcomplex<f32>>, tensor<complex<f32>>) -> tensor<4x5x513xcomplex<f32>>
// CHECK-NEXT:     %21 = stablehlo.pad %20, %cst_1, low = [0, 0, 0], high = [0, 0, 511], interior = [0, 0, 0] : (tensor<4x5x513xcomplex<f32>>, tensor<complex<f32>>) -> tensor<4x5x1024xcomplex<f32>>
// CHECK-NEXT:     %22 = stablehlo.fft %21, type =  FFT, length = [1024] : (tensor<4x5x1024xcomplex<f32>>) -> tensor<4x5x1024xcomplex<f32>>
// CHECK-NEXT:     %23 = stablehlo.real %22 : (tensor<4x5x1024xcomplex<f32>>) -> tensor<4x5x1024xf32>
// CHECK-NEXT:     %24 = stablehlo.reduce(%23 init: %cst_2) applies stablehlo.add across dimensions = [2, 1] : (tensor<4x5x1024xf32>, tensor<f32>) -> tensor<4xf32>
// CHECK-NEXT{LITERAL}:     %25 = stablehlo.convolution(%23, %arg1) dim_numbers = [f, b, 0]x[i, o, 0]->[b, f, 0], window = {stride = [1], pad = [[0, 0]], lhs_dilate = [1], rhs_dilate = [1]} {batch_group_count = 1 : i64, feature_group_count = 1 : i64, precision_config = [#stablehlo<precision DEFAULT>, #stablehlo<precision DEFAULT>]} : (tensor<4x5x1024xf32>, tensor<4x2x1xf32>) -> tensor<5x2x1024xf32>
// CHECK-NEXT{LITERAL}:     %26 = stablehlo.convolution(%arg0, %23) dim_numbers = [f, b, 0]x[o, i, 0]->[f, b, 0], window = {stride = [1], pad = [[0, 0]], rhs_dilate = [1]} {batch_group_count = 1 : i64, feature_group_count = 1 : i64, precision_config = [#stablehlo<precision DEFAULT>, #stablehlo<precision DEFAULT>]} : (tensor<5x2x1024xf32>, tensor<4x5x1024xf32>) -> tensor<4x2x1xf32>
// CHECK-NEXT:     return %25, %26, %24, %17, %arg0, %arg1, %arg2, %arg3 : tensor<5x2x1024xf32>, tensor<4x2x1xf32>, tensor<4xf32>, tensor<16x4x16xcomplex<f32>>, tensor<5x2x1024xf32>, tensor<4x2x1xf32>, tensor<4xf32>, tensor<16x4x16xcomplex<f32>>
// CHECK-NEXT: }
