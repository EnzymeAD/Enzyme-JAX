...
module @reactant_differe... attributes {mhlo.num_partitions = 1 : i64, mhlo.num_replicas = 1 : i64} {
  func.func private @"Const{typeof(estimate_tracer_error)}(Main.estimate_tracer_error)_autodiff"(%arg0: tensor<f64>) -> (tensor<f64>) {
  ...
  func.func @main() {
    ...
    %cst2 = stablehlo.constant dense<2.000000e+00> : tensor<f64>
    
    %0 = enzyme.autodiff @"Const{typeof(estimate_tracer_error)}(Main.estimate_tracer_error)_autodiff"(%cst0, %cst) {activity = [#enzyme<activity enzyme_active>], ret_activity = [#enzyme<activity enzyme_activenoneed>]} : (tensor<f64>, tensor<f64>) -> (tensor<f64>)
...
// CHECK:  func.func private @"diffeConst{typeof(estimate_tracer_error)}(Main.estimate_tracer_error)_autodiff"(%arg0: tensor<f64>, %arg1: tensor<f64>) -> tensor<f64> {
