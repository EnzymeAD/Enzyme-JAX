...
// CHECK-NEXT:      %47 = stablehlo.reshape %46 : (tensor<2x3xf32>) -> tensor<1x2x3xf32>
// CHECK-NEXT:      %48 = stablehlo.dynamic_update_slice %iterArg_15, %47, %iterArg, %c_4, %c_4 : (tensor<5x2x3xf32>, tensor<1x2x3xf32>, tensor<i64>, tensor<i64>, tensor<i64>) -> tensor<5x2x3xf32>
...
// CHECK-NEXT:      %52 = stablehlo.dot_general %iterArg_9, %46, contracting_dims = [1] x [1] : (tensor<3x3xf32>, tensor<2x3xf32>) -> tensor<3x2xf32>
...
// CHECK-NEXT:      stablehlo.return %41, %iterArg_9, %iterArg_10, %58, %iterArg_12, %40, %38, %48, %57 : tensor<i64>, tensor<3x3xf32>, tensor<3x3xf32>, tensor<3x2xf32>, tensor<3x2x6xf32>, tensor<5x3x3xf32>, tensor<5x3x2xf32>, tensor<5x2x3xf32>, tensor<5x3x2xf32>
// CHECK-NEXT:    }
...
// CHECK-NEXT:      %39 = stablehlo.dynamic_slice %12#7, %iterArg_14, %c_4, %c_4, sizes = [1, 2, 3] : (tensor<5x2x3xf32>, tensor<i64>, tensor<i64>, tensor<i64>) -> tensor<1x2x3xf32>
// CHECK-NEXT:      %40 = stablehlo.reshape %39 : (tensor<1x2x3xf32>) -> tensor<2x3xf32>
// CHECK-NEXT:      %41 = stablehlo.dynamic_slice %12#6, %iterArg_14, %c_4, %c_4, sizes = [1, 3, 2] : (tensor<5x3x2xf32>, tensor<i64>, tensor<i64>, tensor<i64>) -> tensor<1x3x2xf32>
// CHECK-NEXT:      %42 = stablehlo.reshape %41 : (tensor<1x3x2xf32>) -> tensor<3x2xf32>
// CHECK-NEXT:      %43 = stablehlo.dynamic_slice %12#5, %iterArg_14, %c_4, %c_4, sizes = [1, 3, 3] : (tensor<5x3x3xf32>, tensor<i64>, tensor<i64>, tensor<i64>) -> tensor<1x3x3xf32>
// CHECK-NEXT:      %44 = stablehlo.reshape %43 : (tensor<1x3x3xf32>) -> tensor<3x3xf32>
// CHECK-NEXT:      %45 = stablehlo.add %iterArg, %c_6 : tensor<i64>
// CHECK-NEXT:      %46 = stablehlo.tanh %38 : tensor<3x2xf32>
...
// CHECK-NEXT:      %52 = stablehlo.dot_general %49, %40, contracting_dims = [1] x [0] : (tensor<3x2xf32>, tensor<2x3xf32>) -> tensor<3x3xf32>
// CHECK-NEXT:      %53 = stablehlo.add %iterArg_9, %52 : tensor<3x3xf32>
// CHECK-NEXT:      %54 = stablehlo.add %iterArg_12, %50 : tensor<3xf32>
// CHECK-NEXT:      %55 = stablehlo.dot_general %49, %42, contracting_dims = [1] x [1] : (tensor<3x2xf32>, tensor<3x2xf32>) -> tensor<3x3xf32>
// CHECK-NEXT:      %56 = stablehlo.add %iterArg_10, %55 : tensor<3x3xf32>
// CHECK-NEXT:      %57 = stablehlo.dot_general %44, %49, contracting_dims = [0] x [0] : (tensor<3x3xf32>, tensor<3x2xf32>) -> tensor<3x2xf32>
