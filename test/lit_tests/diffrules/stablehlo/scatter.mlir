...
// REVERSE-NEXT:    %0 = "stablehlo.scatter"(%arg2, %c, %cst_0) <{indices_are_sorted = true, scatter_dimension_numbers = #stablehlo.scatter<update_window_dims = [0, 1, 2, 3, 4], scatter_dims_to_operand_dims = [2]>, unique_indices = true}> ({
...
// REVERSE-NEXT:    %1 = "stablehlo.gather"(%arg2, %c) <{dimension_numbers = #stablehlo.gather<offset_dims = [0, 1, 2, 3, 4], start_index_map = [2]>, indices_are_sorted = true, slice_sizes = array<i64: 1, 1, 2048, 16, 256>}> : (tensor<1x1x8192x16x256xbf16>, tensor<1xi32>) -> tensor<1x1x2048x16x256xbf16>
// REVERSE-NEXT:    return %0, %1 : tensor<1x1x8192x16x256xbf16>, tensor<1x1x2048x16x256xbf16>
// REVERSE-NEXT:  }
...
// REVERSE2-NEXT:    %0 = "stablehlo.gather"(%arg2, %c) <{dimension_numbers = #stablehlo.gather<offset_dims = [0, 1, 2, 3, 4], start_index_map = [2]>, indices_are_sorted = true, slice_sizes = array<i64: 1, 1, 2048, 16, 256>}> : (tensor<1x1x8192x16x256xbf16>, tensor<1xi32>) -> tensor<1x1x2048x16x256xbf16>
// REVERSE2-NEXT:    return %0 : tensor<1x1x2048x16x256xbf16>
// REVERSE2-NEXT: }
