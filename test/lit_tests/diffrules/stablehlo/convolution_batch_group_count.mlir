...
// REVERSE-NEXT{LITERAL}:    %2 = stablehlo.reverse %1, dims = [0, 1] : tensor<3x3x16x1xf32>
// REVERSE-NEXT{LITERAL}:    %3 = stablehlo.convolution(%arg2, %2) dim_numbers = [b, f, 1, 0]x[0, 1, o, i]->[b, f, 1, 0], window = {stride = [1, 1], pad = [[2, 2], [2, 2]], lhs_dilate = [1, 1], rhs_dilate = [1, 1]} {batch_group_count = 1 : i64, feature_group_count = 4 : i64} : (tensor<2x4x2x2xf32>, tensor<3x3x16x1xf32>) -> tensor<2x16x4x4xf32>
