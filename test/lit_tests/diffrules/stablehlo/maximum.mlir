// RUN: enzymexlamlir-opt %s --enzyme-wrap="infn=maximum outfn= retTys=enzyme_dup argTys=enzyme_dup,enzyme_dup mode=ForwardMode" --enzyme-hlo-opt --cse | FileCheck %s --check-prefix=FORWARD
// RUN: enzymexlamlir-opt %s --enzyme-wrap="infn=maximum outfn= retTys=enzyme_active argTys=enzyme_active,enzyme_active mode=ReverseModeCombined" --canonicalize --remove-unnecessary-enzyme-ops --arith-raise --enzyme-hlo-opt --cse | FileCheck %s --check-prefix=REVERSE
// RUN: enzymexlamlir-opt %s --enzyme --canonicalize --remove-unnecessary-enzyme-ops --arith-raise | stablehlo-translate - --interpret

func.func @maximum(%a : tensor<4xf32>, %b : tensor<4xf32>) -> tensor<4xf32> {
  %c = stablehlo.maximum %a, %b : (tensor<4xf32>, tensor<4xf32>) -> tensor<4xf32>
  func.return %c : tensor<4xf32>
}

// FORWARD:  func.func @maximum(%arg0: tensor<4xf32>, %arg1: tensor<4xf32>, %arg2: tensor<4xf32>, %arg3: tensor<4xf32>) -> (tensor<4xf32>, tensor<4xf32>) {
// FORWARD-NEXT:    %0 = stablehlo.compare  LT, %arg0, %arg2 : (tensor<4xf32>, tensor<4xf32>) -> tensor<4xi1>
// FORWARD-NEXT:    %1 = stablehlo.select %0, %arg3, %arg1 : tensor<4xi1>, tensor<4xf32>
// FORWARD-NEXT:    %2 = stablehlo.maximum %arg0, %arg2 : tensor<4xf32>
// FORWARD-NEXT:    return %2, %1 : tensor<4xf32>, tensor<4xf32>
// FORWARD-NEXT:  }

// REVERSE:  func.func @maximum(%arg0: tensor<4xf32>, %arg1: tensor<4xf32>, %arg2: tensor<4xf32>) -> (tensor<4xf32>, tensor<4xf32>) {
// REVERSE-NEXT:    %cst = stablehlo.constant dense<0.000000e+00> : tensor<4xf32>
// REVERSE-NEXT:    %0 = stablehlo.compare  LT, %arg0, %arg1 : (tensor<4xf32>, tensor<4xf32>) -> tensor<4xi1>
...
func.func @main() {
  ...
  %output = stablehlo.constant dense<[2.0, -1.0, 1.5, 100.0]> : tensor<4xf32>

  %expected_da = stablehlo.constant dense<[0.0, 0.0, 1.0, 1.0]> : tensor<4xf32>
  %expected_db = stablehlo.constant dense<[1.0, 1.0, 0.0, 0.0]> : tensor<4xf32>
