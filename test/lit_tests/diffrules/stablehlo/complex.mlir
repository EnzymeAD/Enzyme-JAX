...
// RUN: enzymexlamlir-opt %s --enzyme-wrap="infn=main outfn= retTys=enzyme_active argTys=enzyme_active,enzyme_active mode=ReverseModeCombined" --arith-raise --verify-each=0 --canonicalize --enzyme-simplify-math --remove-unnecessary-enzyme-ops | FileCheck %s --check-prefix=REVERSE
