...
module {
  ...
  func.func @ckpt(%arg0: tensor<3xf64>) -> tensor<3xf64> {
    ...
    %c_n = stablehlo.constant dense<10> : tensor<i64>
    %c_0 = stablehlo.constant dense<0> : tensor<i64>
    %0:2 = stablehlo.while(%iterArg = %c_0, %iterArg_x = %arg0) : tensor<i64>, tensor<3xf64> attributes {enzyme.disable_mincut, enzyme.enable_checkpointing = true, enzyme.checkpoint_period = 3 : i64}
     cond {
      %1 = stablehlo.compare LT, %iterArg, %c_n : (tensor<i64>, tensor<i64>) -> tensor<i1>
      stablehlo.return %1 : tensor<i1>
    } do {
