...
// REVERSE-NEXT:             %0 = stablehlo.reverse %arg1, dims = [0, 1] : tensor<2x2x1x2xf64>
// REVERSE-NEXT{LITERAL}:    %1 = stablehlo.convolution(%arg2, %0) dim_numbers = [b, f, 1, 0]x[0, 1, o, i]->[b, f, 1, 0], window = {stride = [1, 1], pad = [[0, 0], [0, 0]], lhs_dilate = [1, 1], rhs_dilate = [2, 2]} {batch_group_count = 1 : i64, feature_group_count = 1 : i64} : (tensor<3x2x6x6xf64>, tensor<2x2x1x2xf64>) -> tensor<3x1x4x4xf64>
// REVERSE-NEXT{LITERAL}:    %2 = stablehlo.convolution(%arg0, %arg2) dim_numbers = [f, b, 1, 0]x[i, o, 1, 0]->[0, 1, b, f], window = {stride = [2, 2], pad = [[2, 2], [2, 2]], rhs_dilate = [1, 1]} {batch_group_count = 1 : i64, feature_group_count = 1 : i64} : (tensor<3x1x4x4xf64>, tensor<3x2x6x6xf64>) -> tensor<2x2x1x2xf64>
// REVERSE-NEXT{LITERAL}:    return %1, %2 : tensor<3x1x4x4xf64>, tensor<2x2x1x2xf64>
