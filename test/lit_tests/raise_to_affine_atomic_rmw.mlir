...
// CHECK:             %[[AFFINE_ATOMIC_RMW_0:.*]] = enzyme.affine_atomic_rmw addf %[[ARG0]], %[[ARG1]], (#[[$ATTR_0]]) {{\[}}%[[VAL_0]]] acq_rel : (f32, memref<?xf32>) -> f32
...
module {
  func.func private @use(%arg0: f32)
  // CHECK: #[[$ATTR_0:.+]] = affine_map<(d0) -> (d0)
  // CHECK-LABEL:   func.func @affine3(
  // CHECK-SAME:      %[[ARG0:.*]]: f32,
  // CHECK-SAME:      %[[ARG1:.*]]: memref<?xf32>) {
  func.func @affine3(%arg0: f32, %arg1: memref<?xf32>) {
    // CHECK:           affine.parallel (%[[VAL_0:.*]]) = (0) to (4) {
    affine.parallel (%arg2) = (0) to (4) {
      // CHECK: %[[AFFINE_ATOMIC_RMW_0:.*]] = enzyme.affine_atomic_rmw addf %[[ARG0]], %[[ARG1]], (#[[$ATTR_0]]) {{\[}}%[[VAL_0]]] monotonic {alignment = 4 : i64} : (f32, memref<?xf32>) -> f32
      %0 = enzyme.atomic_rmw addf %arg0, %arg1[%arg2] monotonic {alignment = 4} : (f32, memref<?xf32>) -> f32
      func.call @use(%0) : (f32) -> ()
    }
    return
  }
