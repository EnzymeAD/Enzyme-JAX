...
// CHECK-NEXT:      %23 = arith.index_cast %{{[0-9]+}} : i64 to index
// CHECK-NEXT:      %24 = arith.index_cast %5 : i64 to index
// CHECK-DAG:       %{{[0-9]+}} = arith.index_cast %{{[0-9]+}} : i64 to index
// CHECK-DAG:       %{{[0-9]+}} = arith.addi %{{[0-9]+}}, %{{[0-9]+}} : index
// CHECK-NEXT:      %27 = arith.addi %{{[0-9]+}}, %{{[0-9]+}} : index
...
// CHECK-NEXT:        %58 = arith.index_cast %{{[0-9]+}} : i64 to index
...
// CHECK-NEXT:        %63 = arith.addi %62, %{{[0-9]+}} : index
// CHECK-NEXT:        %64 = arith.subi %63, %59 : index
// CHECK-NEXT:        %65 = arith.subi %64, %{{[0-9]+}} : index
...
// CHECK-NEXT:      %38 = arith.index_cast %{{[0-9]+}} : i64 to index
// CHECK-NEXT:      %39 = "enzymexla.pointer2memref"(%arg0) : (!llvm.ptr<1>) -> memref<?xf64, 1 : index>
// CHECK-NEXT:      %40 = arith.addi %{{[0-9]+}}, %c20169 : index
// CHECK-NEXT:      %41 = arith.subi %40, %37 : index
// CHECK-NEXT:      %42 = arith.subi %41, %{{[0-9]+}} : index
...
// CHECK-DAG:       %{{[0-9]+}} = arith.index_cast %{{[0-9]+}} : i64 to index
// CHECK-DAG:       %{{[0-9]+}} = arith.addi %{{[0-9]+}}, %{{[0-9]+}} : index
// CHECK-NEXT:      %49 = arith.addi %{{[0-9]+}}, %{{[0-9]+}} : index
