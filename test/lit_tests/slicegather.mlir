...
// CHECK-NEXT:   %0 = stablehlo.slice %arg0 [0:2, 0:4] : (tensor<3x4xf32>) -> tensor<2x4xf32>
// CHECK-NEXT:   %1 = stablehlo.slice %arg1 [0:2, 0:5, 0:1] : (tensor<3x5x1xi64>) -> tensor<2x5x1xi64>
// CHECK-NEXT:   %2 = "stablehlo.gather"(%0, %1) <{dimension_numbers = #stablehlo.gather<collapsed_slice_dims = [1], operand_batching_dims = [0], start_indices_batching_dims = [0], start_index_map = [1], index_vector_dim = 2>, indices_are_sorted = false, slice_sizes = array<i64: 1, 1>}> : (tensor<2x4xf32>, tensor<2x5x1xi64>) -> tensor<2x5xf32>
