...
module {
  func.func @main(%arg0: tensor<i64> {enzymexla.memory_effects = ["read", "write", "allocate", "free"]}) -> tensor<i64> attributes {enzymexla.memory_effects = ["read", "write", "allocate", "free"]} {
    ...
    %0 = enzymexla.mpi.allreduce(%arg0, %c, %c_0) {datatype=#enzymexla.datatype<MPI_INT>, op="MPI_SOME_OP"} : (tensor<i64>, tensor<i64>, tensor<i32>) -> tensor<i64>
    return %0 : tensor<i64>
  }
}
...
// CPU-NEXT:      %0 = enzymexla.jit_call @enzymexla_wrapper_MPI_Allreduce_MPI_SOME_OP_MPI_INT (%arg0, %c, %c_0) {output_operand_aliases = [#stablehlo.output_operand_alias<output_tuple_indices = [], operand_index = 1, operand_tuple_indices = []>]} : (tensor<i64>, tensor<i64>, tensor<i32>) -> tensor<i64>
