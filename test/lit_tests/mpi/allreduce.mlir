...
// CPU:  module {
// CPU-NEXT:    llvm.mlir.global external constant @MPI_SOME_OP() {addr_space = 0 : i32} : !llvm.ptr
// CPU-NEXT:    llvm.mlir.global external constant @MPI_SOME_TYPE() {addr_space = 0 : i32} : !llvm.ptr
...
// CPU-NEXT:      %0 = enzymexla.jit_call @enzymexla_wrapper_MPI_Allreduce_MPI_SOME_OP_MPI_SOME_TYPE (%arg0, %c, %c_0) {output_operand_aliases = [#stablehlo.output_operand_alias<output_tuple_indices = [], operand_index = 1, operand_tuple_indices = []>]} : (tensor<i64>, tensor<i64>, tensor<i32>) -> tensor<i64>
