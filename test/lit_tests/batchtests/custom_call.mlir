...
// CHECK-NEXT:       %2 = stablehlo.divide %iterArg, %c : tensor<i64>
// CHECK-NEXT:       %3 = stablehlo.dynamic_slice %arg0, %iterArg, %2, %c_2, %c_2, sizes = [1, 1, 64, 64] : (tensor<2x3x64x64xf32>, tensor<i64>, tensor<i64>, tensor<i64>, tensor<i64>) -> tensor<1x1x64x64xf32>
// CHECK-NEXT:       %4 = stablehlo.reshape %3 : (tensor<1x1x64x64xf32>) -> tensor<64x64xf32>
// CHECK-NEXT:       %5:3 = stablehlo.custom_call @cusolver_getrf_ffi(%4) {api_version = 4 : i32, operand_layouts = [dense<[1, 0]> : tensor<2xindex>], output_operand_aliases = [#stablehlo.output_operand_alias<output_tuple_indices = [0], operand_index = 0, operand_tuple_indices = []>], result_layouts = [dense<[1, 0]> : tensor<2xindex>, dense<0> : tensor<1xindex>, dense<> : tensor<0xindex>]} : (tensor<64x64xf32>) -> (tensor<64x64xf32>, tensor<64xi32>, tensor<i32>)
// CHECK-NEXT:       %6 = stablehlo.reshape %5#0 : (tensor<64x64xf32>) -> tensor<1x1x64x64xf32>
// CHECK-NEXT:       %7 = stablehlo.dynamic_update_slice %iterArg_3, %6, %iterArg, %2, %c_2, %c_2 : (tensor<2x3x64x64xf32>, tensor<1x1x64x64xf32>, tensor<i64>, tensor<i64>, tensor<i64>, tensor<i64>) -> tensor<2x3x64x64xf32>
// CHECK-NEXT:       stablehlo.return %1, %7 : tensor<i64>, tensor<2x3x64x64xf32>
// CHECK-NEXT:     }
