...
func.func @do_while() -> index {
  ...
  %result = scf.while (%i = %c0) : (index) -> index {
    %t1 = "test.test1"(%i) : (index) -> index
    "test.test2"(%i, %t1) : (index, index) -> ()
    %updated = arith.addi %i, %c1 : index
    %t3 = "test.test3"(%updated, %t1) : (index, index) -> index
    "test.test4"(%i, %t1, %t3) : (index, index, index) -> ()
    %cond = arith.cmpi slt, %updated, %c5 : index
    scf.condition(%cond) %updated : index
  } do {
  ^bb0(%new_i: index):
...
// CHECK:           scf.for %[[VAL_3:.*]] = %[[VAL_2]] to %[[VAL_1]] step %[[VAL_0]] {
// CHECK:             %[[VAL_4:.*]] = "test.test1"(%[[VAL_3]]) : (index) -> index
// CHECK:             "test.test2"(%[[VAL_3]], %[[VAL_4]]) : (index, index) -> ()
// CHECK:             %[[VAL_5:.*]] = arith.addi %[[VAL_3]], %[[VAL_0]] : index
// CHECK:             %[[VAL_6:.*]] = "test.test3"(%[[VAL_5]], %[[VAL_4]]) : (index, index) -> index
