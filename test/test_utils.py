...
def _is_bazel_environment():
    """Check if we're running in a Bazel environment."""
    runfiles = os.path.dirname(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    )
    # Bazel creates pypi_* directories for python dependencies
    return os.path.exists(os.path.join(runfiles, "pypi_jax"))


def _find_nvidia_lib_path(package_name: str, lib_subdir: str) -> str | None:
    """Find NVIDIA library path from site-packages (for uv/pip environments)."""
    for site_packages in sys.path:
        if "site-packages" in site_packages:
            nvidia_path = os.path.join(
                site_packages, "nvidia", package_name, lib_subdir
            )
...
def _setup_cuda_paths_uv():
    """Setup CUDA paths for uv/pip environments."""
    import ctypes

    # Find NVIDIA libraries in site-packages
    nvidia_libs = [
        ("cuda_nvcc", "bin", None),  # CUDA compiler
        ("cuda_runtime", "lib", None),
    ...
    for package, subdir, lib_file in nvidia_libs:
        lib_path = _find_nvidia_lib_path(package, subdir)
        if lib_path:
            if subdir == "bin":
                path_additions.append(lib_path)
            else:
                ld_lib_paths.append(lib_path)

            # Pre-load specific libraries to avoid version conflicts
            if lib_file:
                full_lib_path = os.path.join(lib_path, lib_file)
                if os.path.exists(full_lib_path):
                    try:
                        ctypes.cdll.LoadLibrary(full_lib_path)
                    except OSError:
                        pass  # Library may already be loaded or not needed
    ...
    if path_additions:
        ...
        os.environ["PATH"] = ":".join(path_additions) + (
            ":" + current_path if current_path else ""
        )

    # Set CUDA_DIR if we found the nvcc package
    cuda_nvcc_path = _find_nvidia_lib_path("cuda_nvcc", "")
    if cuda_nvcc_path:
        os.environ["CUDA_DIR"] = cuda_nvcc_path
        os.environ["XLA_FLAGS"] = f"--xla_gpu_cuda_data_dir={cuda_nvcc_path}"

    cudnn_path = _find_nvidia_lib_path("cudnn", "")
    if cudnn_path:
        os.environ["CUDNN_PATH"] = cudnn_path


def fix_paths():
    # Clear potentially conflicting NVIDIA environment variables
    for nm in [
    ...
    if not _is_bazel_environment():
        # Running with uv/pip - use site-packages paths
        _setup_cuda_paths_uv()
        return

    # Original Bazel-specific path setup follows
    runfiles = os.path.dirname(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    )
