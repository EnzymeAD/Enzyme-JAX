# /// script
# requires-python = ">=3.12"
# dependencies = [
#     "absl-py",
#     "numpy",
#     "pandas",
#     "tabulate",
#     "seaborn",
#     "matplotlib",
# ]
# ///

import base64
import collections
import glob
import os
import re
import tempfile
import zipfile
from datetime import datetime, timezone

import matplotlib.pyplot as plt
import pandas as pd
import seaborn as sns
from absl import app, flags, logging

_ARTIFACT_DIR = flags.DEFINE_string(
    "artifact_dir", None, "Path to the artifact directory", required=True
)

_MARKDOWN_FILE = flags.DEFINE_string(
    "markdown_file", None, "Path to the markdown file", required=True
)

_CSV_FILE = flags.DEFINE_string("csv_file", None, "Path to the csv file", required=True)

_PLOTS_DIRECTORY = flags.DEFINE_string(
    "plots_directory", None, "Path to the plots directory", required=True
)


def get_machine_name(filename: str) -> str:
    pattern = re.compile(r"benchmark-(.+?)-\d+\.\d+")
    match = pattern.search(filename)
    if match:
        return match.group(1)
    raise ValueError(f"Could not find machine name in filename: {filename}")


def main(_) -> None:
    # unzip the outputs.zip
    tempdir_results = tempfile.TemporaryDirectory(delete=False)
    logging.info("Extracting outputs to %s", tempdir_results.name)

    for file in glob.glob(f"{_ARTIFACT_DIR.value}/**/outputs.zip", recursive=True):
        logging.info("Unzipping %s...", file)
        machine_name = file.strip(f"{_ARTIFACT_DIR.value}/").split("/")[0]

        with zipfile.ZipFile(file, "r") as zip_ref:
            zip_ref.extractall(os.path.join(tempdir_results.name, machine_name))

    # Merge the CSVs into a single dataframe
    combined_dfs = collections.defaultdict(list)
    for file in glob.glob(f"{tempdir_results.name}/*/results_*.csv"):
        machine_name = get_machine_name(os.path.basename(os.path.dirname(file)))
        expt = os.path.basename(file).replace("results_", "").replace(".csv", "")

        df = pd.read_csv(file)
        df["machine"] = machine_name

        combined_dfs[expt].append(df)

    # Merge the CSVs into a single dataframe
    final_dfs = dict()
    for expt, dfs in combined_dfs.items():
        merged = pd.concat(dfs, ignore_index=True)
        final_dfs[expt] = merged

    dfs_merged = pd.concat(final_dfs.values(), ignore_index=True)
    dfs_merged.to_csv(_CSV_FILE.value, index=False)
    logging.info("Saved combined results to %s", _CSV_FILE.value)

    os.makedirs(_PLOTS_DIRECTORY.value, exist_ok=True)

    plot_files = dict()

    # Create a markdown table
    with open(_MARKDOWN_FILE.value, "w") as f:
        f.write("## 🚀 Benchmark Results (Table)\n\n")

        for expt, df in final_dfs.items():
            f.write(f"<details>\n<summary><strong>📊 {expt}</strong></summary>\n\n")
            df = df.drop("Benchmark Name", axis=1)
            df.to_markdown(f, index=False)
            f.write("\n\n</details>\n\n")

            g = sns.catplot(
                data=df,
                x="Key",
                y="Time",
                hue="Pass Pipeline",
                dodge=True,
                kind="bar",
                col="Backend",
                row="machine",
                sharey=False,
                margin_titles=True,
            )
            # g.set(yscale="log")
            plot_file_pdf = os.path.join(_PLOTS_DIRECTORY.value, f"{expt}.pdf")
            plot_file = os.path.join(_PLOTS_DIRECTORY.value, f"{expt}.png")
            g.savefig(plot_file)
            g.savefig(plot_file_pdf)
            logging.info("Saved plot to %s", plot_file)

            plot_files[expt] = plot_file

        f.write(f"## 🚀 Benchmark Results (Plots)\n\n")

        for expt, plot_file in plot_files.items():
            f.write(f"<details>\n<summary><strong>📊 {expt}</strong></summary>\n\n")

            with open(plot_file, "rb") as pf:
                encoded = base64.b64encode(pf.read()).decode("utf-8")

            f.write(f"![{expt} plot](data:image/png;base64,{encoded})\n\n")

            f.write("\n\n</details>\n\n")

        f.write(
            f"_Generated by GitHub Actions on {datetime.now(timezone.utc).isoformat()}_\n"
        )

    logging.info("Created markdown file at %s", _MARKDOWN_FILE.value)


if __name__ == "__main__":
    app.run(main)()
