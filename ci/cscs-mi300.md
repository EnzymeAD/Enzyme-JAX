...
build runs [`local/build.sh`](local/build.sh) inside `srun --uenv`, which first
assembles a writable ROCm **overlay** at `${BUILD_ROOT}/.rocm` (symlinks into the
UENV view + spack package store) that stands in for a normal `ROCM_PATH`, then
invokes Bazel against it.

- `local/build.sh` — builds the overlay + runs the Bazel build. A checked-in file,
  shared verbatim with the local (non-CI) build; see
  [`local/README.md`](local/README.md).
- `run_julia.sh` — generated by the yml; launches Julia with `LD_LIBRARY_PATH`
  pointed at the curated ROCm libs.

### Heredoc levels

The overlay build used to be generated inline from the yml, which stacked three
levels of heredoc escaping and made a `$` mean different things on adjacent lines.
It now lives in [`local/build.sh`](local/build.sh) as a real file, shared verbatim
with the local build (see [`local/README.md`](local/README.md)), so only two levels
remain:

| Level | What runs it | Escape to defer past it |
|---|---|---|
| 0 — `build.sh` | runs inside `srun`, writes the clang wrappers (unquoted `<< WRAPPER`) | `\$` |
| 1 — the clang wrapper | runs per compiler invocation | — |

So in `build.sh`, `${BUILD_ROOT}` and `${OVERLAY_RESOURCE_DIR}` resolve when the
script runs inside `srun`, and `\$@` reaches the wrapper as a literal `$@`.
`build.sh` takes `BUILD_ROOT`, `REACTANT_DIR` and `JULIA` from the environment; the
yml supplies them as job variables (`BUILD_ROOT: "${CI_PROJECT_DIR}"`).

`run_julia.sh` is still generated by the yml, with a **quoted** heredoc
(`<< 'RUNSCRIPT'`), so nothing expands at write time and every var resolves at run
time — that is why its body reads like ordinary shell.
