...
void IslScop::cleanup(Operation *func) {
  func->walk([](enzymexla::AffineStoreVar op) { op->erase(); });
}

SmallVector<Location> getArgumentLocs(Block &block) {
  return llvm::map_to_vector(block.getArguments(),
...
IslScop::applySchedule(__isl_take isl_schedule *newSchedule,
...

  {
    ...
    b.createBlock(&f->getRegion(0), f->getRegion(0).begin(),
                  ...
                  getArgumentLocs(originalFunc->getRegion(0).front()));
...
std::unique_ptr<IslScop> IslScopBuilder::build(Operation *f) {
  ...
  for (auto &stmt : scop->stmts) {
...

    {
      ...
      // value type, temp fix
      if (isa<enzymexla::Pointer2MemrefOp>(op))
        continue;
