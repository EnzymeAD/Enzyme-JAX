...
void dump(const Node &n) {
  if (n.is<Value>())
    llvm::errs() << "[" << n.get<Value>() << ", " << "Value" << "]\n";
  else if (n.is<Operation *>())
    llvm::errs() << "[" << *n.get<Operation *>() << ", " << "Operation"
                 << "]\n";
  else
    llvm::errs() << "[" << "NULL" << ", " << "None" << "]\n";
}
...
static void dump(Graph &G) {
  ...
  struct WhileOpEnzymeOpsRemover
      : public EnzymeOpsRemoverOpInterface::ExternalModel<
            WhileOpEnzymeOpsRemover, stablehlo::WhileOp> {
  private:
      ...
      for (Value newCache : newCacheVec) {
        ...
        while (!worklist.empty()) {
          ...
          int64_t newSize = computeSizeOfType(candidate),
                  newRank =
                      cast<RankedTensorType>(candidate.getType()).getRank();
      ...
      for (Value newCache : newCaches) {
        enzyme::InitOp initOp = ({
          ...
          rewriter.create<enzyme::InitOp>(
              newCache.getLoc(), enzyme::CacheType::get(newCache.getContext(),
                                                        newCache.getType()));
        });
