...
void constructScatterGatherIndices(OpTy op, Value &result,
                                   OpBuilder &builder,
  ...
  SmallVector<int64_t> indexVectorShape = {whileLoopInfo.getConstantNumIters(), 1};
  ...
  for (size_t i = 0; i < startIndices.size(); i++) {
    ...
    Value depIndices =
        stablehlo::IotaOp::create(builder, loc, depIndicesTy, 0);

    // step * iota
    if (!whileLoopInfo.isStepOne()) {
      depIndices = stablehlo::MulOp::create(builder, loc, depIndices, bcastIndex(whileLoopInfo.getStep(builder)));
    }
    ...
    depIndices = stablehlo::AddOp::create(builder, loc, depIndices, offsetConst);
...
bool WhileLoopInfo::hoistOperationFromLoop(
  ...
  Value gatherIndices;
  constructScatterGatherIndices(sliceOp, gatherIndices, builder, dimensions, *this);
...
bool WhileLoopInfo::hoistOperationFromLoop(
  ...
  constructScatterGatherIndices(dusOp, scatterIndices, builder, dimensions, *this);
