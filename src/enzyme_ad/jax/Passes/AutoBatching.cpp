...
bool GreedyWhileLoopBatchFission::liftOperationByBatching(
  ...
  for (auto [consType, baseOp, sliceDim, sliceInfo] : llvm::zip(
    ...
    case BatchLiftingMode::DYNAMIC_SLICE: {
      ...
      SmallVector<int64_t> sliceStarts(newSliceStarts.size(), 0);
      SmallVector<int64_t> sliceStrides(newSliceSizes.size(), 1);
      sliceStrides[sliceDim] = scale;
      SmallVector<int64_t> sliceLimits(DSType.getShape().begin(),
                                       DSType.getShape().end());

      newSlice = stablehlo::SliceOp::create(
