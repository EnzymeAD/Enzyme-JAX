...
struct MPICommRankOpLowering
  ...
  std::string backend;
  size_t ncclCommPtr;
  MPICommRankOpLowering(std::string backend, size_t ncclCommPtr, MLIRContext *context,
  ...
  LogicalResult matchAndRewrite(enzymexla::MPICommRankOp op,
    ...
    if (backend == "cpu") {
      ...
      std::string ncclFuncName = "ncclCommUserRank";
      ...
      // Generate the enzymexla_wrapper_MPI_Comm_rank LLVM function body
      std::string wrapperFunctionName = "enzymexla_wrapper_" + ncclFuncName;
      ...
      if (!moduleOp.lookupSymbol<LLVM::LLVMFuncOp>(wrapperFunctionName)) {
        ...
        rewriter.create<LLVM::CallOp>(
            op.getLoc(), TypeRange{i32Type},
            SymbolRefAttr::get(context, ncclFuncName),
            ValueRange{ncclComm, rankPtr});

        rewriter.create<LLVM::ReturnOp>(op.getLoc(), ValueRange{});
      }

      if (!moduleOp.lookupSymbol<LLVM::LLVMFuncOp>(ncclFuncName)) {
        OpBuilder::InsertionGuard guard(rewriter);
        rewriter.setInsertionPointToStart(moduleOp.getBody());
        ...
        rewriter.create<LLVM::LLVMFuncOp>(op.getLoc(), ncclFuncName,
                                          funcType, LLVM::Linkage::External);
      }
    ...
    if (backend == "cpu") {
      ...
      if (!moduleOp.lookupSymbol<LLVM::LLVMFuncOp>(wrapperFunctionName)) {
        ...
        if (!moduleOp.lookupSymbol<LLVM::LLVMFuncOp>(
                cuStreamSynchronizeFunctionName)) {
          auto cuStreamSynchronizeType =
              LLVM::LLVMFunctionType::get(i32Type, {llvmPtrType}, false);
...
struct LowerEnzymeXLAMPIPass
  ...
  void runOnOperation() override {
    ...
    patterns.add<MPICommRankOpLowering>(backend, ncclCommPtr, context);
    patterns.add<MPICommSizeOpLowering>(backend, ncclCommPtr, context);
    patterns.add<MPIBarrierOpLowering>(backend, ncclCommPtr, context);
    patterns.add<MPISendOpLowering>(backend, ncclCommPtr, context);
    patterns.add<MPIRecvOpLowering>(backend, ncclCommPtr, context);
    patterns.add<MPIIsendOpLowering>(backend, ncclCommPtr, context);
    patterns.add<MPIIrecvOpLowering>(backend, ncclCommPtr, context);
    patterns.add<MPIWaitOpLowering>(backend, ncclCommPtr, context);
    patterns.add<MPIWaitallOpLowering>(backend, ncclCommPtr, context);
    patterns.add<MPIAllreduceOpLowering>(backend, ncclCommPtr, context);
    patterns.add<MPIBcastOpLowering>(backend, ncclCommPtr, context);
