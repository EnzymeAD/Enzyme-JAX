...
public:
  LiftXLAWrapperLoop(MLIRContext *context, DataFlowSolver &aliasSolver)
      : OpRewritePattern<LoopOp>(context), aliasSolver(aliasSolver) {}
  ...
  LogicalResult matchAndRewrite(LoopOp loop,
    ...
    // Expand affine bounds only after establishing that the loop can be
    // lifted. In particular, multi-result lower/upper maps mean max/min,
    // respectively. Other affine loops and accesses are left untouched.
...
struct XLAMegakernelizePass
  ...
  void runOnOperation() override {
    ...
    // Run loop lifting after sequential fusion so a serial loop containing the
    // ping-pong pair becomes a single wrapper before we inspect its body.
    RewritePatternSet loopPatterns(&getContext());
    loopPatterns.add<LiftXLAWrapperLoop<scf::ForOp>,
