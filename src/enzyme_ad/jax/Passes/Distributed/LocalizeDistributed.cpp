...
class LocalizeDistributedRunner {
public:
  ...
  std::unique_ptr<LocalizationPolicy>
  getLocalizationPolicy(MeshComputationOp meshComputation) {
    (void)meshComputation;
    return std::make_unique<NaiveLocalMostPolicy>(/*budget=*/2);
  }
  std::unique_ptr<PartitioningPolicy>
  getPartitioningPolicy(MeshComputationOp meshComputation) {
    (void)meshComputation;
    return std::make_unique<TrivialWholeOpPartitioningPolicy>();
  }

  LogicalResult run(MeshComputationOp meshComputationOp) {
    auto localizationPolicy = getLocalizationPolicy(meshComputationOp);
    auto partitioningPolicy = getPartitioningPolicy(meshComputationOp);
    ...
    for (PlannedLocalization &planned : plannedLocalizations) {
      ...
      Block *rangeBlock = (*splitOr).range;
      ...
      llvm::SmallVector<Value> liveOutsAll(liveOutSet.begin(),
                                           liveOutSet.end());
      ...
      FailureOr<llvm::DenseMap<Value, llvm::SmallVector<Value>>>
          deviceTokenMap =
              buildDeviceTokenMap(hoistedCollectives,
                                  localizedLogicalMesh.getMesh(), n_devices,
                                  builder, meshComputationOp.getLoc());
      ...
      // Live-in/live-out will be captured when handling each specific split
      // range, after mutation boundaries are established.
    }
...
localizeMeshComputation(MeshComputationOp meshComputationOp) {
  LocalizeDistributedRunner runner;
  return runner.run(meshComputationOp);
}
...
struct LocalizeDistributedPass
  ...
  void runOnOperation() override {
    if (failed(localizeMeshComputation(getOperation()))) {
      signalPassFailure();
    }
  }
};
