    ...
    for (auto iv : op.getIVs()) {
      ...
      for (auto U : iv.getUsers()) {
	    ...
	    for (size_t i=0; i<operands.size(); i++) {
	      if (operands[i] != iv) continue;
	      if (i < AL.getAffineMap().getNumDims()) {
		 functionOf |= E.isFunctionOfDim(i);
	      } else {
		 functionOf |= E.isFunctionOfSymbol(i - AL.getAffineMap().getNumSymbols());
	      }
	    }
	    if (functionOf)
	      exprs.push_back(E);
	    ...
	    for (size_t i=0; i<operands.size(); i++) {
		 ...
		 functionOf |= E.isFunctionOfSymbol(i - AS.getAffineMap().getNumSymbols());
	      }
	    }
	    if (functionOf)
	      exprs.push_back(E);
	    ...
	    for (size_t i=0; i<operands.size(); i++) {
		 ...
		 functionOf |= E.isFunctionOfSymbol(i - AI.getIntegerSet().getNumSymbols());
	      }
	    }
	    if (functionOf)
	      exprs.push_back(E);
