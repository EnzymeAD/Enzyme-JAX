...
convertLLVMAllocaToMemrefAlloca(LLVM::AllocaOp alloc, RewriterBase &rewriter,
  ...
  auto atAddr =
      dyn_cast<enzymexla::AtAddrOp>(alloc.getRes().use_begin()->getOwner());
  if (!atAddr)
    return failure();
...
static MemRefVal convertToMemref(PtrVal addr) {
  ...
  auto atAddr = builder.create<enzymexla::AtAddrOp>(
      addr.getLoc(), addr,
...
#undef RIS
    ...
    for (auto &use : v.getUses()) {
      if (auto affineScope = dyn_cast<enzymexla::AffineScopeOp>(use.getOwner())) {
...
void populateRemoveIVPatterns(RewritePatternSet &patterns) {
  patterns.insert<mlir::transform::NormalizeLoop, mlir::transform::RemoveIVs>(patterns.getContext());
}
}
