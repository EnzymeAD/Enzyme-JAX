...
struct AffineToStableHLORaisingPass
  ...
  modOp.getBody()->push_back(newFunc);
  SymbolTable::setSymbolVisibility(newFunc, SymbolTable::Visibility::Private);

  {
    OpBuilder builder(g);
    auto newCall = builder.create<enzymexla::XLAWrapperOp>(g->getLoc(), SymbolRefAttr::get(newFunc), llvm::to_vector(operands));
    g->erase();
    anyRaised = true;
  }
