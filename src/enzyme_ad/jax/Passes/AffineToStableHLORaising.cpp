  ...
  auto getMaskedCond = [&](Value cond, Value mask) {
    if (mask) {
      ...
      affine::AffineValueMap newMaskMap =
          alignMemoryAccess(mask, maskMap, cond, condMap, builder, pc, mapping);
...
tryRaisingOpToStableHLO(Operation *op, IRMapping &mapping, OpBuilder &builder,
  ...
  if (auto storeOp = dyn_cast<affine::AffineStoreOp>(op)) {
    ...
    if (pc.mask) {
      ...
      // or different?
      storeValueMap = alignMemoryAccess(mask, maskMap, vals, dsts, builder, pc, mapping);
    ...
    if (needPad) {
      ...
      operand = stablehlo::PadOp::create(
          builder,
          rewriteLocation(op->getLoc(), pc.options.strip_llvm_debuginfo),
          cast<RankedTensorType>(operand.getType()).clone(paddedShape), operand,
          padVal, padLow, padHigh, interior);
    }
