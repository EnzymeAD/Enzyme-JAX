...
matchAndRewrite(scf::ParallelOp parallelOp,
  ...
  auto reductionOp = dyn_cast<scf::ReduceOp>(parallelOp.getBody()->getTerminator());
  if (!reductionOp) {
    return failure();
  }
  ...
  if (parallelCount < 2) return failure();
  ...
  for (auto [iv, lower, upper, step] :
	  ...
	  scf::ForOp forOp = rewriter.create<scf::ForOp>(loc, lower, upper, step, iterArgs);
