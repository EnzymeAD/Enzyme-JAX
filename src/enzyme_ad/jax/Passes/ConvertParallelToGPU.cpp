...
*/
 ...
 SymbolTableCollection symbolTable;
    ...
    std::string sm;
    if (auto attr = dyn_cast_or_null<ArrayAttr>(err->getAttr("passthrough"))) {
    for (auto a : attr) {
    ...
    std::string feat;
    if (auto attr = dyn_cast_or_null<LLVM::TargetFeaturesAttr>(err->getAttr("target_features"))) {
      feat = attr.getFeaturesString();
    }

    err->walk([&](gpu::LaunchFuncOp launch) {
		   auto gfunc = dyn_cast_or_null<gpu::GPUFuncOp>(symbolTable.lookupNearestSymbolFrom(launch, launch.getKernel()));
		  if (!gfunc) return;
		 auto gmod = cast<gpu::GPUModuleOp>(gfunc->getParentOp());
	 	if (!gmod.getTargetsAttr()) {
		  auto chip = sm;
		  if (chip.size() == 0) chip = "sm_50";
		  auto features = feat;
		  if (features.size() == 0) features = "+ptx60";
		  auto target = NVVM::NVVMTargetAttr::get(gmod.getContext(), /*optLevel*/2, /*triple*/"nvptx64-nvidia-cuda", chip, features);
