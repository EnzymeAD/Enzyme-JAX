...
namespace {
struct CommRemoveDeadMessages : public CommRemoveDeadMessagesBase<CommRemoveDeadMessages> {

  static bool is_live(CommMessage msg, llvm::DenseSet<CommMessage> &live_set, llvm::DenseSet<CommMessage> &dead_set){
    if (live_set.contains(msg)) return true;
    if (dead_set.contains(msg)) return false;
    ...
    op->walk([&](CommMessage msg){
      if(!is_live(msg, live_messages, dead_messages)){
        for(auto user: llvm::to_vector(msg->getUsers())){
          // Erase only the sends- the other messages need to be walked over so their own cleanup triggers
          if(isa<CommSend>(user)){
            user->erase();
          }
        }
        // Todo: with multiple messages, will this cause exceptions when we go to erase those?
        msg->erase();
      }
    });
  }
...
std::unique_ptr<Pass> createCommRemoveDeadMessagesPass() {
  return std::make_unique<CommRemoveDeadMessages>();
}
} // namespace enzyme
} // namespace mlir
