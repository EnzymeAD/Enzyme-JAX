...
struct WhileToForHelper {
  ...
  WhileToForHelper(WhileOp loop, CmpIOp cmpIOp, bool cmpNegated = false) : loop(loop), cmpIOp(cmpIOp), cmpNegated(cmpNegated) {
  ...
  bool checkPredicate(PatternRewriter &rewriter) {
    ...
    case CmpIPredicate::ult: {
      ...
      if (stepInt < 0) {
        rewriter.notifyMatchFailure(loop, "Cmp less than with negative step unhandled");
        return false;
      } else if (stepInt == 0) {
    ...
    case CmpIPredicate::sge: {
      ...
      if (stepInt > 0) {
        rewriter.notifyMatchFailure(loop, "Cmp less than with positive step unhandled");
        return false;
      } else if (stepInt == 0) {
  ...
  void prepareFor(PatternRewriter &rewriter) {
    ...
    if (negativeStep) {
      auto zero = rewriter.create<arith::ConstantOp>(step.getLoc(), step.getType(), rewriter.getZeroAttr(step.getType()));
...
struct MoveWhileToFor : public OpRewritePattern<WhileOp> {
  ...
  LogicalResult matchAndRewrite(WhileOp loop,
    ...
    WhileToForHelper helper(loop, condOp.getCondition().getDefiningOp<CmpIOp>());
    Value lookThrough = nullptr;
    if (helper.cmpIOp) {
      if (!helper.computeLegality(rewriter, /*sizeCheck*/ true,
                                  lookThrough, /*doWhile*/ true)) {
        return failure();
      }
      assert(helper.lb);
      assert(helper.ub);
      ...
      if (!legal) {
        return rewriter.notifyMatchFailure(loop, "No legal and comparison found");
      }
    } else {
    ...
    for (Value arg : condOp.getArgs()) {
      forArgs.push_back(
          rewriter.create<ub::PoisonOp>(arg.getLoc(), arg.getType()));
    }

    // And finally, optionally, the extra condition from the and
    if (lookThrough) {
      forArgs.push_back(
          rewriter.create<arith::ConstantIntOp>(loop.getLoc(), rewriter.getI1Type(), true));
    }
    ...
    if (lookThrough) {
      ...
      for (size_t i=0; i<loop.getNumResults(); i++) {
        toYield.push_back(forloop.getRegionIterArgs()[i + loop.getInits().size()]);
      }

      toYield.push_back(rewriter.create<arith::ConstantIntOp>(loop.getLoc(), rewriter.getI1Type(), false));
...
struct RotateWhileAnd : public OpRewritePattern<WhileOp> {
  ...
  LogicalResult matchAndRewrite(WhileOp loop,
    ...
    if (auto andIOp = condOp.getCondition().getDefiningOp<AndIOp>()) {
      for (int i = 0; i < 2; i++) {
        WhileToForHelper helper(loop, andIOp->getOperand(i).getDefiningOp<CmpIOp>());
        if (!helper.cmpIOp)
          continue;
        ...
        if (helper.computeLegality(rewriter, /*sizeCheck*/ false, lookThrough, /*doWhile*/true)) {
    ...
    while (todo.size()) {
      ...
      if (!helper.computeLegality(rewriter, /*sizeCheck*/ false, /*lookThrough*/nullptr, /*doWhile*/true)) {
        if (negated) {
          negatedValues.push_back(operand);
        } else {
          andValues.push_back(operand);
      ...
      for (auto aval : negatedValues) {
        aval = rewriter.create<XOrIOp>(loop.getLoc(), aval, rewriter.create<arith::ConstantIntOp>(aval.getLoc(), aval.getType(), 1));
        if (current == nullptr) {
          current = aval;
        } else {
          current = rewriter.create<AddIOp>(loop.getLoc(), current, aval);
      ...
      if (negated) {
        cmpIOp2 = rewriter.create<CmpIOp>(loop.getLoc(), invertPredicate(cmpIOp.getPredicate()), cmpIOp.getLhs(), cmpIOp.getRhs());
      }
      if (current)
      ...
      rewriter.modifyOpInPlace(condOp, [&]() {
        condOp.getConditionMutable().assign(current);
      });
      return success();
    }
...
void CanonicalizeFor::runOnOperation() {
  ...
  rpl.add<IfYieldMovementPattern, truncProp, ForOpInductionReplacement,
          ...
          RemoveInductionVarRelated, RotateWhileAnd,
