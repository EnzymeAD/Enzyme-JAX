...
struct SliceDUSToConcat final : OpRewritePattern<stablehlo::SliceOp> {
    ...
    LogicalResult matchAndRewrite(stablehlo::SliceOp sliceOp,
        ...
        auto dusOp = sliceOp.getOperand().getDefiningOp<stablehlo::DynamicUpdateSliceOp>();
        if (!dusOp) {
        ...
        auto sliceResultType = dyn_cast<RankedTensorType>(sliceOp.getType()); // Type of %283

        if (!targetType || !updateType || !sliceResultType ||
            !targetType.hasStaticShape() || !updateType.hasStaticShape() || !sliceResultType.hasStaticShape()) {
        ...
        for (Value idxVal : dusIndexVals) {
            ...
            if (!matchPattern(idxVal, m_Constant(&idxAttr)) || idxAttr.getNumElements() != 1) {
                return rewriter.notifyMatchFailure(sliceOp, "DUS indices must be constant scalars");
            }
            dusStartIndices.push_back((*idxAttr.begin()).getSExtValue());
        }

        if (llvm::any_of(sliceOp.getStrides(), [](int64_t s){ return s != 1; })) {
             return rewriter.notifyMatchFailure(sliceOp, "Requires slice strides of 1");
        }
        ...
        if (sliceOp.getStartIndices()[concatDim] < dusStartIndices[concatDim]) {
          ...
          SmallVector<int64_t> newLimit = llvm::to_vector(sliceOp.getLimitIndices());
          newLimit[concatDim] = dusStartIndices[concatDim];
          toConcat.push_back(
            rewriter.create<stablehlo::SliceOp>(sliceOp.getLoc(),
                dusOp.getOperand(),
        ...
        if (sliceOp.getStartIndices()[concatDim] > dusStartIndices[concatDim]) {
          ...
          SmallVector<int64_t> newStart = llvm::to_vector(sliceOp.getStartIndices());
  ...
  void runOnOperation() override {
    ...
    if (passses & 512)
      patterns.add<TransposeDotReorder, DotTranspose, ConvolutionTranspose,
                   ...
                   ReshapeIota, DUSDUS, DUSDUSConcat, DUSConcat, DUSPad, SliceDUSToConcat>(
          context);
