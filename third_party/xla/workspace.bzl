...
def repo(extra_patches = [], override_commit = ""):
    ...
    http_archive(
        ...
        patches = ["//:patches/xla.patch", "//:patches/xla_win.patch", "//:patches/xla_dedup_comm.patch", "//:patches/xla_multipad.patch", "//:patches/xla_bump_stablehlo.patch", "//:patches/xla_recognize_reduce_window.patch"],
        patch_args = ["-p1"],
    )
